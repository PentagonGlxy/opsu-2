...
public class MainMenu extends BasicGameState {
	...
	public void render(GameContainer container, StateBasedGame game, Graphics g)
		...
		// seasonal background: takes priority, since enabling the option is an
		// explicit user choice. getImage() returns null when the option is off,
		// the folder is empty, or the last decode failed, in which case this
		// falls through to the original dynamic background behaviour.
		Image seasonalBg = SeasonalBackground.getImage();
		if (seasonalBg != null) {
			// Aspect-fill, so arbitrary user image sizes are never stretched:
			// scale by whichever axis needs the most coverage and let the other
			// overflow off screen.
			...
			// NOTE: a scaled copy shares its parent's texture, and
			// Image.destroy() cascades to parentImage, so this must never be
			// destroyed here. SeasonalBackground owns the parent's lifetime.
	...
	public void enter(GameContainer container, StateBasedGame game)
		...
		// seasonal background: rescan on every menu entry, so images copied into
		// the folder while the game was running are picked up without a restart.
		// invalidate() clears the "folder was empty" short circuit; reroll() is a
		// cheap no-op when the option is disabled.
	...
	public void leave(GameContainer container, StateBasedGame game)
		...
		// seasonal background: release the texture on the way out. Backgrounds
		// are full screen (~8 MB as RGBA8888 at 1080p), and this state is not
		// visible again until enter() reloads one.
	...
	private void nextTrack(boolean user) {
		...
		// seasonal background: pick a new image on track change, mirroring how
		// the dynamic background follows the track.
	...
	private void previousTrack() {
		if (!previous.isEmpty()) {
			...
			lastMeasureProgress = 0f;
			// seasonal background: same reroll as nextTrack().
			if (Options.isSeasonalBackgroundEnabled()) {
				SeasonalBackground.reroll();
				bgAlpha.setTime(0);
			}
